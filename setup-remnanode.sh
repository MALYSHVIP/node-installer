...
services:
  remnanode:
    volumes:
      ...
      - $STATS_SERVICE_OVERRIDE:/opt/app/dist/src/modules/stats/stats.service.js:ro
...
write_stats_service_override() {
  log "Записываю override stats.service.js с безопасным fallback для online-count"

  docker run --rm --entrypoint cat "$NODE_IMAGE" /opt/app/dist/src/modules/stats/stats.service.js > "$STATS_SERVICE_OVERRIDE"
...
child_process_import = 'const child_process_1 = require("child_process");\n'
import_marker = 'const xtls_sdk_1 = require("@remnawave/xtls-sdk");\n'
if child_process_import not in text:
...
stats_replacement = """getLocalIpv4s() {
        try {
            ...
            this.logger.warn(`Failed to get local IPv4 list for conntrack fallback: ${error}`);
            return new Set();
        }
    }
    buildSyntheticUserIdFromIp(ip, index) {
        try {
            const ipNum = ip.split('.').reduce((acc, part) => (acc << 8n) + BigInt(Number(part)), 0n);
            return (9000000000000000n + ipNum).toString();
        }
        catch {
            return (9000000000000000n + BigInt(index)).toString();
        }
    }
    buildConntrackFallbackUsers() {
        try {
            const localIps = this.getLocalIpv4s();
            if (localIps.size === 0) {
                return [];
            }
            const conntrackBinary = (0, child_process_1.execFileSync)('/bin/sh', [
                '-lc',
                'command -v conntrack || { if command -v apk >/dev/null 2>&1; then apk add --no-cache conntrack-tools >/dev/null 2>&1; fi; command -v conntrack; }',
            ], { encoding: 'utf8' }).trim();
            if (!conntrackBinary) {
                return [];
            }
            const output = (0, child_process_1.execFileSync)(conntrackBinary, ['-L', '-p', 'udp', '--dport', '443'], { encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
            const peerIps = new Set();
            for (const line of output.split('\\n')) {
                const match = line.match(/\\bsrc=(\\S+)\\s+dst=(\\S+)\\s+sport=\\d+\\s+dport=443\\b/);
                if (!match) {
                    continue;
                }
                const [, srcIp, dstIp] = match;
            ...
            return Array.from(peerIps).map((ip, index) => ({
                email: this.buildSyntheticUserIdFromIp(ip, index),
                ips: [{ ip, lastSeen: nowSeconds }],
            }));
        }
        catch (error) {
            this.logger.warn(`Conntrack on-demand fallback probe failed: ${error}`);
            return [];
        }
    }
    async getUsersStats(reset) {
        const getOnlineUsersFallback = async () => {
            try {
                const { users } = await this.xtlsSdk.stats.rawClient.getAllOnlineUsers({});
                const uniqueUsers = Array.from(new Set((users || [])
                    .map((stat) => this.extractOnlineUserId(stat))
                ...
                this.logger.warn(`Failed to get online users fallback: ${error}`);
                return [];
            }
        };
        const getConntrackUsersFallback = () => this.buildConntrackFallbackUsers().map((user) => ({
            username: user.email,
            uplink: 1,
            downlink: 0,
        }));
        const mergeUsers = (primaryUsers, fallbackUsers) => {
            if (fallbackUsers.length === 0) {
                return primaryUsers;
            }
            const seen = new Set(primaryUsers.map((user) => String(user.username)));
        ...
        const alignToConntrackCount = (primaryUsers, conntrackUsers) => {
            if (conntrackUsers.length <= primaryUsers.length) {
                return { users: primaryUsers, added: 0 };
            }
            const seen = new Set(primaryUsers.map((user) => String(user.username)));
            const alignedUsers = [...primaryUsers];
            let added = 0;
            for (const fallbackUser of conntrackUsers) {
                const username = String(fallbackUser.username);
                if (seen.has(username)) {
                    continue;
                }
                seen.add(username);
                alignedUsers.push(fallbackUser);
                added += 1;
                if (alignedUsers.length >= conntrackUsers.length) {
                    break;
                }
            }
            return { users: alignedUsers, added };
        };
        try {
            const response = await this.xtlsSdk.stats.getAllUsersStats(reset);
            const activeUsers = response.isOk && response.data
                ? response.data.users.filter((user) => user.uplink !== 0 || user.downlink !== 0)
                : [];
            const onlineFallbackUsers = await getOnlineUsersFallback();
            let mergedUsers = mergeUsers(activeUsers, onlineFallbackUsers);
            if (activeUsers.length === 0 && onlineFallbackUsers.length > 0) {
                this.logger.warn(`Using online users fallback for ${onlineFallbackUsers.length} connected users`);
            }
            const conntrackFallbackUsers = getConntrackUsersFallback();
            const { users: alignedUsers, added: addedFromConntrack } = alignToConntrackCount(mergedUsers, conntrackFallbackUsers);
            mergedUsers = alignedUsers;
            if (addedFromConntrack > 0) {
                this.logger.warn(`Using conntrack count alignment for ${addedFromConntrack} missing connected users (${mergedUsers.length} total)`);
            }
            ...
            return {
                isOk: true,
                response: new models_1.GetUsersStatsResponseModel([]),
            };
        }
        catch (error) {
            this.logger.error(error);
            const onlineFallbackUsers = await getOnlineUsersFallback();
            let mergedUsers = onlineFallbackUsers;
            if (onlineFallbackUsers.length > 0) {
                this.logger.warn(`Using online users fallback after exception for ${onlineFallbackUsers.length} connected users`);
            }
            const conntrackFallbackUsers = getConntrackUsersFallback();
            const { users: alignedUsers, added: addedFromConntrack } = alignToConntrackCount(mergedUsers, conntrackFallbackUsers);
            mergedUsers = alignedUsers;
            if (addedFromConntrack > 0) {
                this.logger.warn(`Using conntrack count alignment after exception for ${addedFromConntrack} missing connected users (${mergedUsers.length} total)`);
            }
            if (mergedUsers.length > 0) {
                return {
                    isOk: true,
...
ip_list_pattern = re.compile(r"async getUsersIpList\(\) \{.*?^\s+extractOnlineUserId\(raw\) \{", re.S | re.M)
ip_list_replacement = """async getUsersIpList() {
        const alignToConntrackCount = (primaryUsers, conntrackUsers) => {
            if (conntrackUsers.length <= primaryUsers.length) {
                return { users: primaryUsers, added: 0 };
            }
            const knownIps = new Set(primaryUsers.flatMap((user) => Array.isArray(user.ips)
                ? user.ips.map((entry) => entry.ip).filter(Boolean)
                : []));
            const alignedUsers = [...primaryUsers];
            let added = 0;
            for (const fallbackUser of conntrackUsers) {
                const fallbackIp = fallbackUser?.ips?.[0]?.ip;
                if (!fallbackIp || knownIps.has(fallbackIp)) {
                    continue;
                }
                knownIps.add(fallbackIp);
                alignedUsers.push(fallbackUser);
                added += 1;
                if (alignedUsers.length >= conntrackUsers.length) {
                    break;
        ...
        try {
            ...
            if (onlineUsersResponse?.timeout) {
                const conntrackFallbackUsers = this.buildConntrackFallbackUsers();
                if (conntrackFallbackUsers.length > 0) {
                    this.logger.warn(`Using conntrack on-demand IP fallback after Xray timeout for ${conntrackFallbackUsers.length} connected users`);
                    return {
                        isOk: true,
                        response: new models_1.GetUsersIpListResponseModel(conntrackFallbackUsers),
                    };
                }
                return {
                    isOk: true,
                    response: new models_1.GetUsersIpListResponseModel([]),
                };
            }
            const { users } = onlineUsersResponse;
            const onlineUsers = Array.from(new Set((users || []).map((stat) => this.extractOnlineUserId(stat))));
            ...
            const usersWithIps = usersIps.filter((user) => user.ips.length > 0);
            if (usersIps.length > 0 && usersWithIps.length !== usersIps.length) {
                this.logger.warn(`Using online-only fallback for ${usersIps.length - usersWithIps.length} connected users without IP telemetry`);
            }
            const conntrackFallbackUsers = this.buildConntrackFallbackUsers();
            const { users: alignedUsers, added: addedFromConntrack } = alignToConntrackCount(usersIps, conntrackFallbackUsers);
            if (addedFromConntrack > 0) {
                this.logger.warn(`Using conntrack count alignment for ${addedFromConntrack} missing IP users (${alignedUsers.length} total)`);
            }
            if (alignedUsers.length > 0) {
                return {
                    isOk: true,
                    response: new models_1.GetUsersIpListResponseModel(alignedUsers),
                };
            }
            return {
                isOk: true,
            ...
            if (error && typeof error === 'object' && 'code' in error && error.code === 5) {
                const conntrackFallbackUsers = this.buildConntrackFallbackUsers();
                if (conntrackFallbackUsers.length > 0) {
                    this.logger.warn(`Using conntrack on-demand IP fallback after Xray code 5 for ${conntrackFallbackUsers.length} connected users`);
                    return {
                        isOk: true,
                        response: new models_1.GetUsersIpListResponseModel(conntrackFallbackUsers),
                    };
            ...
            const conntrackFallbackUsers = this.buildConntrackFallbackUsers();
            if (conntrackFallbackUsers.length > 0) {
                this.logger.warn(`Using conntrack on-demand IP fallback after exception for ${conntrackFallbackUsers.length} connected users`);
                return {
                    isOk: true,
                    response: new models_1.GetUsersIpListResponseModel(conntrackFallbackUsers),
                };
