...
TLS_RENEW_HOOK="/etc/letsencrypt/renewal-hooks/deploy/remnanode-tls-sync.sh"
...
PANEL_API_INSECURE="${PANEL_API_INSECURE:-0}"
SECRET_VALUE=""
SECRET_SOURCE="unknown"
PANEL_API_LAST_ERROR=""
APT_UPDATED=0
REBOOT_REQUIRED=0
REBOOT_SCHEDULED=0
...
normalize_panel_api_url() {
  ...
  case "$url" in
    http://*|https://*)
      ;;
    *)
      if [[ "$url" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}(:[0-9]+)?(/.*)?$ ]] || [[ "$url" =~ ^localhost(:[0-9]+)?(/.*)?$ ]]; then
        url="http://$url"
      elif [[ "$url" =~ :[0-9]+(/.*)?$ ]]; then
        url="http://$url"
      else
...
validate_panel_api_url() {
  local url="${1:-}"

  [[ -n "$url" ]] || die "PANEL_API_URL пустой"
  [[ "$url" != *" "* ]] || die "PANEL_API_URL содержит пробелы: $url"
  [[ "$url" =~ ^https?://[^[:space:]]+$ ]] || die "PANEL_API_URL должен начинаться с http:// или https://: $url"
}

build_panel_keygen_url() {
  local base_url="${1:-}"
  ...
  case "$base_url" in
    */api/keygen)
      ...
      ;;
    */api)
      printf '%s/keygen' "$base_url"
      ;;
    *)
      printf '%s/api/keygen' "$base_url"
      ;;
  esac
}

parse_panel_api_pubkey() {
  local payload="${1:-}"
  local value=""
...
for candidate in candidates:
    if not isinstance(candidate, dict):
        continue
    for key in ("pubKey", "secretKey", "secret", "key", "value"):
...
' 2>/dev/null || true
  ...
  if [[ -z "$value" ]]; then
    value="$(
      printf '%s\n' "$payload" | sed -nE \
        -e 's/.*"pubKey"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/p' \
        -e 's/.*"secretKey"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/p' \
        | head -n1
    )"
  fi
...
default_panel_api_url() {
  [[ -n "$PANEL_IP" ]] || die "PANEL_IP пустой, не могу собрать URL API панели"
  printf 'http://%s:3000' "$PANEL_IP"
}

fetch_secret_from_panel_api() {
  local keygen_url=""
  local response_file=""
  local stderr_file=""
  local http_code=""
  ...
  [[ -n "$PANEL_API_TOKEN" ]] || {
    PANEL_API_LAST_ERROR="Не задан PANEL_API_TOKEN"
    return 1
  }

  if [[ -z "$PANEL_API_URL" ]]; then
    PANEL_API_URL="$(default_panel_api_url)"
  fi

  PANEL_API_URL="$(normalize_panel_api_url "$PANEL_API_URL")"
  validate_panel_api_url "$PANEL_API_URL"
  keygen_url="$(build_panel_keygen_url "$PANEL_API_URL")"

  response_file="$(mktemp)"
  stderr_file="$(mktemp)"

  if bool_is_true "$PANEL_API_INSECURE"; then
    http_code="$(
      curl -k -sS \
        --connect-timeout 10 \
        --max-time 30 \
  ...
  if [[ ! "$http_code" =~ ^2[0-9][0-9]$ ]]; then
    PANEL_API_LAST_ERROR="$(
      {
        printf 'HTTP %s' "${http_code:-000}"
        if [[ -s "$stderr_file" ]]; then
          printf ': %s' "$(tail -n1 "$stderr_file" | tr -d '\r')"
        elif [[ -n "$response_body" ]]; then
          printf ': %s' "$(printf '%s' "$response_body" | tr '\n' ' ' | sed 's/[[:space:]]\+/ /g' | cut -c1-220)"
        fi
      } | sed 's/[[:space:]]\+$//'
    )"
  ...
  SECRET_VALUE="$(parse_panel_api_pubkey "$response_body")"
  rm -f "$response_file" "$stderr_file"

  if [[ -z "$SECRET_VALUE" ]]; then
    PANEL_API_LAST_ERROR="API ответил без SECRET_KEY/pubKey"
    return 1
  fi

  SECRET_SOURCE="panel_api"
  log "SECRET_KEY получен через API мастер-панели"
  printf "Panel API URL: %s\n" "$PANEL_API_URL"
  printf "NODE_PORT=%s\n" "$NODE_PORT"
  printf "SECRET_KEY=<hidden>, длина=%s\n" "${#SECRET_VALUE}"
  return 0
}

read_panel_api_token() {
  local token_input=""
  ...
  printf "\nВставь API-токен мастер-панели.\n"
  printf "Если оставить пустым, скрипт попросит SECRET_KEY вручную.\n"
  token_input="$(ask "PANEL_API_TOKEN (optional): ")"
  token_input="$(printf '%s' "$token_input" | tr -d '\r' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')"

  if [[ -z "$token_input" ]]; then
    return 1
  fi

  PANEL_API_TOKEN="$token_input"
  return 0
}
...
validate_generated_xhttp_sync_script() {
  ...
  grep -Fq 'try_files \$uri \$uri/ /index.html;' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял экранирование \$uri"
...
write_xhttp_sync_artifacts() {
  ...
  local q_xhttp_domain=""
  local q_xhttp_path=""
  local q_xhttp_h3_enabled=""
  local q_h3_upstream_mode=""
  local q_full_site=""
  local q_h3_site=""

  printf -v q_xhttp_domain '%q' "$XHTTP_DOMAIN"
  printf -v q_xhttp_path '%q' "$XHTTP_PATH"
  printf -v q_xhttp_h3_enabled '%q' "$xhttp_h3_enabled"
  printf -v q_h3_upstream_mode '%q' "$h3_upstream_mode"
  printf -v q_full_site '%q' "$full_site"
  printf -v q_h3_site '%q' "$h3_site"

  cat > "$XHTTP_SYNC_SCRIPT" <<EOF
#!/usr/bin/env bash
set -Eeuo pipefail

XHTTP_DOMAIN=$q_xhttp_domain
XHTTP_PATH=$q_xhttp_path
XHTTP_ENABLE_H3=$q_xhttp_h3_enabled
XHTTP_H3_UPSTREAM_MODE=$q_h3_upstream_mode
FULL_SITE=$q_full_site
H3_SITE=$q_h3_site
EOF
...
case "$(printf '%s' "$XHTTP_ENABLE_H3" | tr '[:upper:]' '[:lower:]')" in
  1|y|yes|true|on)
    h3_runtime_enabled="1"
    ;;
  0|n|no|false|off)
    h3_runtime_enabled="0"
    ;;
...
send_timeout 10m;
tcp_nodelay on;
ssl_session_cache shared:TOKER_SSL:50m;
ssl_session_timeout 1d;
...
EOF
  ...
  sync_node_tls_artifacts
  write_tls_renew_hook
  write_xhttp_sync_artifacts
  validate_generated_xhttp_sync_script
  ...
  log "xHTTP H2 socket обнаружен: ${h2_socket}"
  if [[ "$xhttp_h3_enabled" == "auto" ]]; then
    log "xHTTP H3 будет включаться автоматически только если появится отдельный h3 socket"
  fi
...
sync_node_tls_artifacts() {
  ...
  src_dir="/etc/letsencrypt/live/$XHTTP_DOMAIN"
  [[ -f "$src_dir/fullchain.pem" ]] || die "Не найден $src_dir/fullchain.pem"
  [[ -f "$src_dir/privkey.pem" ]] || die "Не найден $src_dir/privkey.pem"
  [[ -f "$src_dir/cert.pem" ]] || die "Не найден $src_dir/cert.pem"

  install -d -m 0755 "$(dirname "$NODE_TLS_DIR")" "$NODE_TLS_DIR"
  cp -fL "$src_dir/fullchain.pem" "$NODE_TLS_DIR/fullchain.pem"
  cp -fL "$src_dir/privkey.pem" "$NODE_TLS_DIR/privkey.pem"
  cp -fL "$src_dir/cert.pem" "$NODE_TLS_DIR/cert.pem"
  cp -fL "$src_dir/privkey.pem" "$NODE_TLS_DIR/cert.key"
  chmod 0644 "$NODE_TLS_DIR/fullchain.pem" "$NODE_TLS_DIR/cert.pem"
  chmod 0600 "$NODE_TLS_DIR/privkey.pem" "$NODE_TLS_DIR/cert.key"
}
...
write_tls_renew_hook() {
  ...
  install -d -m 0755 "$(dirname "$TLS_RENEW_HOOK")"
  cat > "$TLS_RENEW_HOOK" <<EOF
#!/usr/bin/env bash
set -Eeuo pipefail
...
cp -fL "\$SRC/privkey.pem" "\$DST/cert.key"
chmod 0644 "\$DST/fullchain.pem" "\$DST/cert.pem"
chmod 0600 "\$DST/privkey.pem" "\$DST/cert.key"
docker compose -f "$COMPOSE_FILE" -f "$COMPOSE_OVERRIDE_FILE" up -d --force-recreate remnanode >/dev/null
EOF

  chmod 0755 "$TLS_RENEW_HOOK"
}
...
read_secret_from_env() {
  ...
  secret_raw="${SECRET_KEY:-${SSL_CERT:-}}"
  ...
  SECRET_SOURCE="env"
  log "SECRET_KEY взят из переменной окружения"
  printf "NODE_PORT=%s\n" "$NODE_PORT"
  printf "SECRET_KEY=<hidden>, длина=%s\n" "${#SECRET_VALUE}"
  return 0
}
...
read_manual_secret() {
  ...
  printf "NODE_PORT=%s\n" "$NODE_PORT"
  printf "SECRET_KEY=<hidden>, длина=%s\n" "${#SECRET_VALUE}"
  SECRET_SOURCE="manual"
}

read_secret() {
  local effective_panel_api_url=""

  if read_secret_from_env; then
    return 0
  fi

  if read_panel_api_token; then
    effective_panel_api_url="${PANEL_API_URL:-$(default_panel_api_url)}"
    if fetch_secret_from_panel_api; then
      return 0
    fi

    if [[ -n "$PANEL_API_LAST_ERROR" ]]; then
      log "Не удалось получить SECRET_KEY через API панели: $PANEL_API_LAST_ERROR"
    else
      log "Не удалось получить SECRET_KEY через API панели"
    fi
    log "Использованный URL API: $effective_panel_api_url"
    log "Если у панели другой адрес или другой API path, передай PANEL_API_URL заранее."
    log "Перехожу к ручному вводу SECRET_KEY"
  fi
...
services:
  remnanode:
    volumes:
      ...
      - $NODE_TLS_DIR:$NODE_TLS_DIR:ro
      - $NODE_TLS_DIR:/var/lib/remnawave/configs/xray/ssl:ro
EOF
}
...
show_result() {
  ...
  printf "SECRET_KEY источник: %s\n" "$SECRET_SOURCE"
  printf "IP ноды и MTU определяются автоматически, руками нужен PANEL_IP и либо SECRET_KEY, либо PANEL_API_URL + PANEL_API_TOKEN.\n"
