...
validate_generated_xhttp_sync_script() {
  ...
  grep -Fq 'grpc_set_header Connection "";' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял очистку Connection header"
  grep -Fq 'grpc_set_header X-Forwarded-Host \$host;' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял X-Forwarded-Host"
  grep -Fq 'grpc_set_header X-Forwarded-Port \$server_port;' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял X-Forwarded-Port"
  grep -Fq 'client_body_buffer_size 1m;' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял client_body_buffer_size"
  grep -Fq 'http2_max_concurrent_streams 1024;' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял HTTP/2 stream limit"
  grep -Fq 'http2_body_preread_size 128k;' "$XHTTP_SYNC_SCRIPT" || die "xHTTP sync script потерял HTTP/2 body preread"
...
read_manual_secret() {
  ...
  if [[ -n "${SECRET_KEY:-}" ]]; then
    ...
    log "Секрет ноды принят из переменной окружения"
    printf "NODE_PORT=%s\n" "$NODE_PORT"
  ...
  if [[ -f "$ENV_FILE" ]]; then
    secret_raw="$(sed -nE \
      -e 's/^[[:space:]]*SECRET_KEY=(.*)$/\1/p' \
      -e 's/^[[:space:]]*SSL_CERT=(.*)$/\1/p' \
      "$ENV_FILE" | head -n1)"

    if [[ -n "$secret_raw" ]]; then
      SECRET_VALUE="$(parse_secret "$secret_raw")"
      if [[ -n "$SECRET_VALUE" ]]; then
        log "Использую существующий секрет ноды из $ENV_FILE"
        printf "NODE_PORT=%s\n" "$NODE_PORT"
        printf "SECRET_KEY=<hidden>, длина=%s\n" "${#SECRET_VALUE}"
        return 0
      fi
