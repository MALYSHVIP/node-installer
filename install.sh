...
Полезные переменные окружения:
  ...
  PANEL_API_TOKEN              API-токен панели для автополучения SECRET_KEY
  PANEL_API_URL                Необязательный override URL панели, по умолчанию http://PANEL_IP:3000
  PANEL_API_INSECURE           1/true/y если нужен curl -k для панели с self-signed TLS
  NODE_IP                      Можно заранее передать IPv4 ноды
  SECRET_KEY                   Можно заранее передать секрет ноды и пропустить API/manual prompt
